...
Default ID type: `number` (auto-incrementing integers). Supports uuidv7 IDs (new in 3.83) for globally sortable unique identifiers.
...
Replace the default filesystem upload storage. All storage adapters support `useCompositePrefixes` (new in 3.83) for simplified key handling with signed URLs and composite prefixes.
...
export default buildConfig({
  email: nodemailerAdapter({
    ...
    // New in 3.84: override recipients per-email
    recipientOverride: (to) => to,  // function to transform recipients
  }),
})
```
```
...
export default buildConfig({
  email: resendAdapter({
    ...
    // New in 3.83: custom headers for Resend adapter
    headers: {
      'X-Entity-RefId': 'my-entity-ref',
    },
  }),
})
```
...
### definePlugin Helper (New in 3.83)

The `definePlugin` helper introduces opt-in execution ordering, cross-plugin discovery via a slug-keyed `plugins` map, and module augmentation for type-safe plugin options. The existing `(config) => config` contract remains unchanged.
...
export const myPlugin = definePlugin<MyPluginOptions>({
  slug: 'my-plugin',
  order: 5,  // lower numbers execute first
  plugin: ({ config, plugins, collections, generateTitle }) => ({
    ...config,
    // access other plugins by slug
    seoConfig: plugins['plugin-seo']?.options,
    collections: [
      ...(config.collections || []),
      {
        slug: 'my-plugin-docs',
...
- **Execution ordering** — `order` property controls which plugins run first (lower = earlier)
- **Cross-plugin discovery** — `plugins` map keyed by slug for type-safe inter-plugin communication
- **Module augmentation** — TypeScript module declaration for typed plugin options
- **Collections access** — `collections` parameter provides already-defined collections
- **generateTitle helper** — reusable title generation function

### MCP Plugin (New in 3.83)

The official MCP (Model Context Protocol) plugin enables AI agent integration with Payload. Supports server instructions and external plugin extension.
...
plugins: [
  mcpPlugin({
    ...
    instructions: 'You are a CMS assistant. Help users manage content.',
  }),
]
```

External plugins can extend the MCP plugin to expose additional tools and resources.
