...
Returns verbatim drawer content with both `cosine=` and `bm25=` scores (v3.3.4+), so users see which component of the match is firing. Uses the same `_hybrid_rank` as the MCP `mempalace_search` tool. Optional filters: `--wing`, `--room`, `--results N`.

Legacy palaces created before `hnsw:space=cosine` was consistently set now show a one-line warning pointing at `mempalace repair`. Without cosine, similarity display floored every result to `Match: 0.0`.
