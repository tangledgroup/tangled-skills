...
category: compiler
external_references:
  - https://github.com/numba/numba/tree/0.65.1
  - https://github.com/numba/numba/releases/tag/0.65.1
...
- **`sys.monitoring` disabled on Python 3.14.4+** — CPython 3.14.4 changed interpreter internals that Numba relied on for JIT `sys.monitoring` integration ([#10538](https://github.com/numba/numba/issues/10538)). The feature is disabled from 3.14.4 onward; `NUMBA_ENABLE_SYS_MONITORING` has no effect and emits a `UserWarning` if set to a non-zero value.
