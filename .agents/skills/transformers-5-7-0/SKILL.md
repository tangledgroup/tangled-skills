...
external_references:
  - https://github.com/huggingface/transformers/tree/v5.7.0/docs
...
- **Laguna XS.2** — Poolside's MoE model with per-layer head counts and sigmoid MoE router (auxiliary-loss-free load balancing)
- **DEIMv2** — Real-time object detection extending DEIM with DINOv3 features, 8 sizes from X to Atto, Spatial Tuning Adapter for larger variants
- **CPU request offloading** for continuous batching generation
- Fixed AutoTokenizer regression that caused wrong tokenizer class initialization (affected DeepSeek R1 and others)
- Fixed Qwen 3.5/3.6 MoE FP8 support, flash attention path, cross-attention cache types
- Improved kernel configuration reading and error handling for FP8 checkpoints

### New in 5.6.x

- **OpenAI Privacy Filter** — Bidirectional PII detection/masking model with 8 privacy categories
- **QianfanOCR** — 4B parameter end-to-end document intelligence model by Baidu (image-to-text, table extraction, chart understanding)
- **SAM3-LiteText** — Lightweight SAM3 variant with MobileCLIP-based text encoder (replaces 353M param encoder via knowledge distillation)
