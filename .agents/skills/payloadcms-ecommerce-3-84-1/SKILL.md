...
**Plugin Configuration Reference**: Full ecommerce plugin config options, collection overrides, access control patterns, locale-aware currency formatting → [Plugin Configuration](reference/01-plugin-configuration.md)
...
**Orders and Transactions**: Order lifecycle, transaction tracking, Stripe webhooks with PaymentIntent verification, guest order access with accessToken → [Orders and Transactions](reference/04-orders-and-transactions.md)
