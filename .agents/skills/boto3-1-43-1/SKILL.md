...
external_references:
  - https://github.com/boto/boto3/tree/1.43.1
...
# Boto3 1.43.1 — AWS SDK for Python
...
Version 1.43.1 requires Python 3.10 or later. Python 3.9 support has ended. It depends on:
...
### What's New in 1.43.x

**1.43.0 highlights:**
- End of support for Python 3.9
- CloudFront cache tags — tag objects via response headers, invalidate all matching objects in single request
- Bedrock AgentCore batch evaluation with server-side orchestration, AI-powered recommendations, and AB testing
- Bedrock AgentCore configuration bundles (versioned, immutable snapshots with branch-based lineage)
- ECR removed support for registry policy V1
- GameLift Servers added DescribeContainerGroupPortMappings API for container fleets
- Transfer Family increased remote directory listing limits from 10k to 200k entries
- WorkSpaces Web added IPv6 range configuration on IP Access Settings
- AWS CRT updated to 0.32.2

**1.43.1 highlights:**
- Bedrock AgentCore Identity: on-behalf-of token exchange OAuth2 support
- Bedrock AgentCore Memory: metadata for LongTerm Memory Records
- DataZone: asynchronous notebook runs support
- EKS: vended logs update param for capability feature
- Kafka (MSK): ZookeeperAccess field for Client-Zookeeper connectivity control
- Observability Admin: enablement for AWS Kafka, Bedrock AgentCore Workload Identity, and OTel metric enablement
- Payment Cryptography: resource-based policies on keys (cross-account sharing), Multi-Party Approval (MPA) team association APIs
- Route 53 Global Resolver: regions support in UpdateGlobalResolver input
- SageMaker: InstancePools for flexible endpoint provisioning, Specifications for InferenceComponent per-instance-type configs
- SSO Admin: InstanceArn and IdentityStoreArn in CreateApplication/DescribeApplication responses
