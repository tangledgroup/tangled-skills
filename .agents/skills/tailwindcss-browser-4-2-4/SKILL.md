...
### Version 4.2.1–4.2.4 Changes

Patch-level compiler fixes (no API changes):

- **4.2.1** — trailing dash in functional utility names for backwards compatibility; better `.` detection in MDX curly braces
- **4.2.2** — Vite 8 support; crash fix for prototype-property candidates (`row-constructor`); `calc(var(--spacing)*…)` canonicalization; `@property` at-rule crash fix
- **4.2.3** — improved `tracking-*` canonicalization; invalid unicode candidate crash fix; arbitrary-value collapse into shorthand utilities
- **4.2.4** — Vite alias resolution fix for `@import`/`@plugin`
