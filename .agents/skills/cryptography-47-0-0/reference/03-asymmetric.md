...
**PKCS1v15 without DigestInfo (since 47.0.0):** For interoperability with legacy systems, use `NoDigestInfo`:

```python
from cryptography.hazmat.primitives.asymmetric.utils import NoDigestInfo

signature = private_key.sign(b"message", PKCS1v15(digest_info=NoDigestInfo()), hashes.SHA256())
```
...
Supported curves: SECP256R1 (P-256), SECP384R1 (P-384), SECP521R1 (P-521).

**Note**: Binary elliptic curves (`SECT*`) were removed in 47.0.0 due to security concerns and rarity of use.
...
**Important (since 47.0.0)**: Loading keys with unsupported algorithms or unsupported explicit curve encodings now raises `cryptography.exceptions.UnsupportedAlgorithm` instead of `ValueError`. Elliptic curve private keys with incorrectly encoded wrong-length keys are also rejected.
...
**Note**: Since 47.0.0, `public_bytes` and `private_bytes` raise `TypeError` (instead of `ValueError`) if an invalid encoding is provided for the given format.
...
### SSH Key Fingerprints (since 45.0.0)

```python
from cryptography.hazmat.primitives.serialization import ssh_key_fingerprint

fingerprint = ssh_key_fingerprint(public_key, "sha256")
```
