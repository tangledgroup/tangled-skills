...
- `.fixed` positioning modifier for `x-anchor` — anchors stay fixed during scroll
- Minified ESM build output for each package

### Fixed

- CSP evaluator: support `MemberExpression` assignments (e.g., `obj.prop = val`)
- CSP evaluator: `&&` and `||` now short-circuit correctly
- `x-for` no longer crashes on null-prototype objects (e.g., `Object.groupBy()` results)
- `x-sort` crash when container has a `<template>` with no `.content`
- `x-id` evaluation order so `$id` resolves consistently inside `x-data`
- `@teleport` no longer leaks detached nodes on every Livewire render
