...
ty avoids false positives in untyped code by using `Unknown`. As of v0.0.30, ty no longer unions `Unknown` into most inferred types of unannotated attributes:
...
reveal_type(Foo().value)  # revealed: int (not Unknown | int)
Foo().value = "x"  # error: [invalid-assignment]
```

Initializers of `None` and other non-literal singleton types remain exceptions:
