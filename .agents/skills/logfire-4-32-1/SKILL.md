...
**Concepts Deep Dive**: Spans, traces, metrics, logs with examples → See [Concepts](reference/01-concepts.md)

**Manual Tracing**: Spans, attributes, messages, f-strings, exceptions, log levels → See [Manual Tracing](reference/02-manual-tracing.md)

**Auto-Tracing**: Automatic function-level tracing with module filtering and duration thresholds → See [Auto-Tracing](reference/03-auto-tracing.md)

**Integrations**: Web frameworks, databases, HTTP clients, LLMs, task queues, logging libraries → See [Integrations](reference/04-integrations.md)

**AI Observability**: LLM panels, token tracking, cost monitoring, tool call inspection, evaluations → See [AI Observability](reference/05-ai-observability.md)

**Metrics**: Counters, histograms, up-down counters, gauges, callback metrics → See [Metrics](reference/06-metrics.md)

**Configuration**: Programmatic, environment variables, pyproject.toml, multiple configs → See [Configuration](reference/07-configuration.md)

**Sampling**: Head sampling, tail sampling by level/duration, combined strategies → See [Sampling](reference/08-sampling.md)

**Distributed Tracing**: Context propagation, thread/pool executors, cross-service traces → See [Distributed Tracing](reference/09-distributed-tracing.md)

**SQL Querying**: Records table schema, columns, JSON operators, time bucketing → See [SQL Reference](reference/10-sql-reference.md)

**Scrubbing**: Sensitive data redaction, custom patterns, callbacks, security tips → See [Scrubbing](reference/11-scrubbing.md)

**Alternative Backends**: Jaeger, OTel Collector, environment variable configuration → See [Alternative Backends](reference/12-alternative-backends.md)

**JavaScript SDK**: Browser, Next.js, Cloudflare Workers, Express, Node.js, Deno → See [JavaScript SDK](reference/13-javascript-sdk.md)

**MCP Server**: Remote MCP for LLM access to telemetry data → See [MCP Server](reference/14-mcp-server.md)

**Self-Hosted**: Kubernetes deployment, Helm chart, system requirements → See [Self-Hosted](reference/15-self-hosted.md)
