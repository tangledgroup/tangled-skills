...
- Block-level suppression comments (`# ruff: disable[...]` / `# ruff: enable[...]`)
...
- Suppressing violations with `noqa` comments or block-level suppressions
...
ruff check @arguments.txt           # Lint using an input file.
```
...
**Linter Deep Dive**: Rule selection, fix safety, error suppression mechanisms, block-level suppressions → [The Ruff Linter](reference/01-the-ruff-linter.md)

**Formatter Deep Dive**: Black compatibility, 2026 style guide, Markdown code formatting, configuration options → [The Ruff Formatter](reference/02-the-ruff-formatter.md)

**Configuration Reference**: File discovery, hierarchical config, CLI flags, extension mapping → [Configuring Ruff](reference/03-configuring-ruff.md)
