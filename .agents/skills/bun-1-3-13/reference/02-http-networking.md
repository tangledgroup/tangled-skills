...
Bun.serve supports HTTP Range requests for partial content delivery, useful for file downloads and streaming:
...
const server = Bun.serve({
  ...
  fetch(req) {
    const url = new URL(req.url);
    if (url.pathname === "/large-file.bin") {
      const file = Bun.file("./large-file.bin");
      // Bun automatically handles Range headers and returns 206 Partial Content
      return file.stream();
    }
    return new Response("Not Found", { status: 404 });
  },
...
```ts
const res = await fetch("http://localhost:3000/large-file.bin", {
  headers: { Range: "bytes=0-1023" },
});
console.log(res.status); // 206
console.log(res.headers.get("Content-Range")); // "bytes 0-1023/1048576"
```
...
// Unix domain socket WebSocket (v1.3.13+)
const unixWs = new WebSocket("ws+unix:///path/to/socket/ws");
