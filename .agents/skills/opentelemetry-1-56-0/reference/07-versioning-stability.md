...
### Current Signal Statuses (1.56.0)

- **Traces**: Stable (Tracer `enabled` API and `AlwaysRecord` sampler stabilized in 1.56.0)
- **Metrics**: Mixed (API and Data Model are Stable; SDK has Development components; Prometheus exporter transformations stabilized in 1.56.0)
- **Logs**: Stable (`Exception` parameter to Emit LogRecord stabilized in 1.55.0)
- **Baggage**: Stable
- **Profiles**: Development
- **Entities**: Development (merge algorithm defined in 1.56.0)
- **Context**: Stable (Environment Variables as Context Propagation Carriers promoted to Beta in 1.56.0)
...
- **OpenTracing compatibility** deprecated in 1.56.0 — OpenTracing compatibility requirements are now deprecated in the specification
...
- Spec changes must now consider the declarative configuration schema
- Strict YAML parsing guidance added to supplementary guidelines
- In-development guidance for exposing the effective `Resource` returned by `Create`
