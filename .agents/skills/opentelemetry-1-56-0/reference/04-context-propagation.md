...
## Environment Variables as Context Propagation Carriers (Beta in 1.56.0)

Environment variables can serve as carriers for propagating context across process boundaries. This is useful for process spawning scenarios where HTTP headers are not available.

### Changes in 1.56.0 (Promoted to Beta)

- **POSIX.1-2024 aligned name restrictions**: Environment variable context propagation names now follow POSIX.1-2024 naming conventions with defined normalization behavior
- **Decoupled carrier responsibilities**: The environment variable propagation carrier's responsibilities are decoupled, separating concerns more clearly
- **Removed misleading implementation approach**: Previous guidance on implementation approaches that could mislead implementers was removed
- **Document status**: Promoted to Beta (from Development)

### Usage

When spawning child processes, context can be propagated via environment variables instead of HTTP headers. The carrier reads/writes from/to the process environment, allowing traces and baggage to follow across process boundaries without network protocols.
