...
|----|-------|-------------|
| 1 | a2a-1-0-0 | Open standard for communication between independent AI agent systems. Agents discover capabilities via Agent Cards... |
| 2 | acp-0-12-2 | Protocol standardizing communication between code editors and AI coding agents using JSON-RPC 2.0 over stdio.... |
...
| 4 | agent-coding-wyattdave-0-1-0 | A toolkit for creating custom AI coding agents using prompt engineering, instruction files, and skill modules. Use... |
...
| 6 | agent-ralph-loop-claude | Implements the Ralph Wiggum technique as a Claude Code plugin for iterative, self-referential AI development loops.... |
| 7 | agent-ralph-loop-vercel-labs | Continuous autonomy framework for the Vercel AI SDK implementing the Ralph Wiggum technique — an iterative outer... |
| 8 | agent-ralph-wiggum-fstandhartinger-0-3-0 | Autonomous AI coding with spec-driven development combining iterative bash loops and SpecKit-style specifications... |
...
| 11 | agent-ralph-wiggum-snarktank | Autonomous AI coding loop that runs AI coding tools (Amp or Claude Code) repeatedly until all PRD items are... |
...
| 14 | aiohttp-3-13-0 | Async HTTP client/server for Python asyncio. Use when building async web applications, REST APIs, or making... |
| 15 | aiohttp-3-13-5 | Async HTTP client/server for Python asyncio. Use when building async web applications, REST APIs, or making... |
...
| 18 | aiohttp-security-0-5-0 | Authentication and authorization toolkit for aiohttp.web applications providing identity policies (cookies,... |
...
| 24 | alpinejs-3-15-12 | A skill for building reactive user interfaces with Alpine.js 3.15, a minimal JavaScript framework that uses... |
...
| 27 | authelia-4-39-19 | Open-source authentication and authorization server providing multi-factor authentication (MFA), single sign-on... |
...
| 31 | axios-1-15-2 | A comprehensive toolkit for making HTTP requests using Axios 1.x, a promise-based HTTP client for browser and... |
...
| 35 | caddy-2-11-2 | Complete Caddy 2.11.2 web server toolkit covering Caddyfile configuration, JSON API, automatic HTTPS with Let's... |
...
| 37 | caveman-1-5-1 | Ultra-compressed communication mode that cuts token usage by ~75% while maintaining full technical accuracy.... |
| 38 | cffi-2-0-0 | Python C Foreign Function Interface for calling C libraries from Python with C-like declarations. Use when... |
...
| 41 | check-jsonschema-0-37-2 | CLI tool for validating JSON, YAML, TOML, and JSON5 files against JSON Schema. Use when verifying configuration... |
| 42 | chibi-scheme-0-12 | Minimal Scheme implementation for embedding in C applications. Provides tagged-pointer VM with precise non-moving... |
...
| 44 | coding-guidelines-0-1-0 | Behavioral guidelines to reduce common LLM coding mistakes including overcomplication, hidden assumptions,... |
...
| 47 | cryptography-47-0-0 | Comprehensive toolkit for Python cryptographic operations using the cryptography library v47.0.0. Use when... |
...
| 49 | cython-3-2-4 | A skill for using Cython 3.2.4, an optimizing Python compiler that makes writing C extensions as easy as Python... |
| 50 | d3-7-9-0 | JavaScript library for bespoke data-driven documents providing SVG and Canvas-based visualizations with scales,... |
| 51 | daisyui-5-5-19 | A skill for using DaisyUI 5.5.19, a component library for Tailwind CSS 4 that provides semantic class names for... |
...
| 56 | dirac-0-2-89 | 'Open-source coding agent that reduces API costs by 64.8% while maintaining 100% accuracy on complex refactoring... |
...
| 58 | docxjs-0-3-6 | Renders DOCX documents into semantic HTML in the browser using JavaScript. Use when converting Word documents to... |
...
| 60 | duckdb-1-5-2 | High-performance analytical SQL database with support for nested types, vectorized execution, and seamless... |
...
| 66 | gh-2-92-0 | GitHub CLI v2.92.0 for managing repositories, pull requests, issues, releases, workflows, codespaces, and more from... |
...
| 68 | git | 'Complete Git 2.54.0 toolkit covering version control workflows, Conventional Commits v1.0.0, Keep a Changelog... |
| 69 | grafana-13-0-1 | Grafana 13.0.1 observability platform for querying, visualizing, alerting on, and exploring metrics, logs, traces,... |
...
| 72 | hnswlib-0-9-0 | Header-only C++ library implementing HNSW graphs for fast approximate nearest neighbor search with Python bindings... |
| 73 | htm-3-1-1 | A skill for using htm 3.1, a tagged template syntax library that provides JSX-like markup in plain JavaScript... |
...
| 76 | hy-1-2-0 | Lisp dialect embedded in Python providing prefix S-expression syntax, compile-time macros, quasiquoting, reader... |
| 77 | jina-ai-reader | Converts any URL to LLM-friendly markdown via r.jina.ai and searches the web with s.jina.ai, returning top results... |
...
| 82 | lambda-calculus-and-lisp-2026-05-12 | Covers the relationship between lambda calculus and Lisp, including McCarthy's EVAL, Church encodings in Scheme, the... |
...
| 85 | lisp-in-python-2026-05-03 | Build a minimal Lisp interpreter in Python covering the eval-apply cycle, recursive descent parsing, lexical... |
...
| 88 | logfire-4-32-1 | AI observability platform built on OpenTelemetry by the Pydantic team. Native SDKs for Python,... |
| 89 | logic | Comprehensive reference covering formal logic systems from propositional through higher-order logic, plus modal... |
...
| 93 | mem0-2-0-1 | Self-improving memory layer for LLM agents (Mem0 v2.0.1) enabling persistent context across sessions with... |
| 94 | mempalace-3-3-4 | Local AI memory system that mines projects and conversations into a searchable index using ChromaDB for vector... |
| 95 | mermaid-11-15-0 | Complete toolkit for Mermaid 11.15.0 providing 29 diagram types including flowcharts, sequence diagrams, Gantt... |
...
| 97 | nekovm-2-4-1 | Stack-based bytecode VM and dynamically typed scripting language, primarily as a compilation target for Haxe.... |
...
| 102 | nltk-3-9-4 | Complete toolkit for Natural Language Processing with NLTK 3.9.4, covering tokenization, stemming, lemmatization,... |
| 103 | nodejs-24-14-0 | Complete Node.js 24.14 runtime toolkit covering core modules, async programming, HTTP servers, file system... |
...
| 109 | oat-0-6-0 | Ultra-lightweight semantic UI component library (~8KB CSS + JS, zero dependencies) that styles HTML elements by... |
| 110 | obscura-0-1-1 | Lightweight headless browser engine written in Rust for web scraping and AI agent automation. Runs JavaScript via... |
...
| 115 | opentelemetry-collector-1-56-0 | Vendor-agnostic telemetry collector for OpenTelemetry providing configurable pipelines for traces, metrics, and... |
...
| 118 | pacote-21-5-0 | Inspect, download, and extract npm packages without installing them first using pacote 21.5 via npx. Supports... |
...
| 120 | pandoc-3-9-0-2 | Universal document converter supporting 50+ input formats (Markdown, HTML, LaTeX, docx, EPUB, Org, RST, AsciiDoc)... |
...
| 124 | payloadcms-blank-3-84-1 | Minimal Payload CMS 3.84.1 starter template providing a clean foundation for building custom headless CMS... |
| 125 | payloadcms-ecommerce-3-84-1 | Payload CMS ecommerce template providing production-ready online store with products, variants, carts, orders,... |
| 126 | payloadcms-website-3-84-1 | Payload CMS website template providing production-ready starter for content-driven websites, blogs, and portfolios... |
...
| 129 | picocss-2-1-1 | Minimalist CSS framework that styles semantic HTML elements by default with responsive typography, automatic... |
| 130 | pinecone-router-7-5-0 | Client-side routing toolkit for Alpine.js applications using Pinecone Router v7.5, providing route matching,... |
| 131 | pipe | Unix-style pipe expression syntax for chaining multiple agent operations sequentially. Each stage's output becomes... |
...
| 136 | podman-py-5-8-0 | Python client library for Podman container engine providing programmatic access to containers, images, pods,... |
...
| 139 | prescheme-2026-05-03 | Statically typed Scheme dialect that compiles to C via Hindley/Milner type inference and CPS transformations.... |
| 140 | prometheus-3-11-3 | Complete toolkit for Prometheus 3.11.3 covering configuration, PromQL querying, service discovery, rules, alerting,... |
...
| 148 | python-prometheus-0-25-0 | Official Prometheus Python client library v0.25.0 for instrumenting Python applications with metrics. Provides... |
...
| 150 | pytorch-2-12-0 | Complete toolkit for PyTorch 2.12 providing n-dimensional tensors, automatic differentiation, neural network... |
| 151 | pywebview-6-2-1 | Cross-platform wrapper around native webview components that lets Python applications display HTML content in a... |
...
| 158 | raft-2026-05-03 | Raft consensus algorithm for replicated log management across server clusters. Covers leader election, log... |
| 159 | ralph-orchestrator-2-9-2 | Orchestration framework that keeps AI agents in a loop until tasks are done. Supports Claude Code, Gemini CLI,... |
...
| 164 | rsync-3-4-2 | Complete toolkit for rsync 3.4.2, the fast file-copying tool using delta-transfer algorithm. Use when backing up... |
...
| 166 | ruff-0-15-12 | Extremely fast Python linter and code formatter written in Rust with 900+ rules, block-level suppressions, and... |
| 167 | rustfs-1-0-0-beta-1 | High-performance distributed object storage system with S3-compatible API, OpenStack Swift support, and... |
...
| 169 | s-expression-alternatives | Alternative syntaxes for Lisp-family languages that reduce or eliminate parentheses while preserving homoiconicity... |
...
| 175 | semver-2-0-0 | Implement and validate Semantic Versioning 2.0.0 to manage software version numbers, determine compatibility,... |
| 176 | sentence-transformers-5-4-1 | Comprehensive toolkit for computing text embeddings, semantic search, and reranking using Sentence Transformers... |
...
| 178 | skman | Meta skill for authoring and managing agent skills. Generates spec-compliant, cross-platform SKILL.md files from... |
...
| 180 | solid-router-0-16-1 | The universal router for SolidJS providing fine-grained reactivity for route navigation with support for... |
...
| 184 | spec-kit-0-8-3 | Spec-Driven Development toolkit for specification-first workflows with AI agents. Generate executable... |
...
| 186 | sqlite-3-53-0 | Embedded SQL database providing ACID transactions, full-text search (FTS5), spatial indexing (R-Tree), JSON... |
...
| 188 | stitch-2026-05-04 | AI-powered UI design tool generating high-fidelity screens and frontend code from text prompts and images. Provides... |
...
| 191 | tailwindcss-4-2-4 | A skill for using Tailwind CSS v4.2, a utility-first CSS framework with CSS-based configuration via @theme, OKLCH... |
...
| 198 | tinypy-1-1-0 | Minimalist Python implementation in ~64k of code with a bootstrapped parser, bytecode compiler, and VM with... |
...
| 202 | ty-0-0-33 | Extremely fast Python type checker and language server written in Rust, 10x-100x faster than mypy and Pyright.... |
...
| 205 | usearch-2-25-1 | High-performance single-file similarity search engine for vectors using HNSW with user-defined metrics,... |
| 206 | uv-0-11-19 | Manages Python projects, tools, scripts, and environments using uv 0.11.19 — Astral's extremely fast Python package... |
...
| 208 | vega-embed-7-1-0 | 'Embed interactive Vega and Vega-Lite visualizations into web pages. Load specs from URLs or JSON objects, render... |
| 209 | vega-lite-6-4-3 | 'Vega-Lite 6.4.3 — high-level grammar of interactive graphics. Generate, author, and debug Vega-Lite specifications... |
...
| 212 | webfetch | Fetches any URL and converts the full page to clean AI-targeted markdown using scrapling. Use when scraping... |
| 213 | websearch | Searches DuckDuckGo via its HTML endpoint and outputs results as raw YAML. Uses a deterministic bash script,... |
...
| 216 | zeromq-wiki-3-2-0 | A comprehensive toolkit for ZeroMQ (ØMQ) messaging library covering socket patterns, protocols, architecture, and... |
| 217 | zeromq-zguide-3-2-0 | Complete ZeroMQ ZGuide 3.2 toolkit covering messaging patterns, socket types, reliability mechanisms, and... |
